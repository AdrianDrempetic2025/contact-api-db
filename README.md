...
├── src/
│   ├── db/
│   │   ├── client.ts         # Drizzle ORM client and DB connection
│   │   ├── schema.ts         # Database schema definitions
│   │   ├── migrate.ts        # Migration application script
│   │   └── seed.ts           # Seed script for example data
│   ├── lib/
│   │   ├── api/
│   │   │   └── errorToResponse.ts  # Centralized error mapping utility
│   │   ├── env.ts            # Environment variable loader
│   │   └── validateContact.ts # Input validation utility (legacy)
│   ├── modules/
│   │   └── contacts/
│   │       ├── handler/
│   │       │   └── handleCreateContact.ts  # API handler logic (legacy)
│   │       ├── model/
│   │       │   └── contact.schema.ts       # Zod schema for contact input
│   │       └── service/
│   │           ├── createContact.ts        # Service layer for contact creation
│   │           └── dummy.ts.md              # Dummy service for testing
├── config/
│   └── constants.ts           # Shared constants and validation rules
├── docs/                     # Extensive documentation and phase reports
